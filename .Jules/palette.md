...
## 2026-06-26 - [Form Label Accessibility and Hitbox]
**Learning:** Naked `<label>` elements without `htmlFor` fail to provide programmatic context to screen readers and miss the opportunity to increase the clickable hitbox for inputs.
**Action:** Always link `<label>` elements to their corresponding inputs using `htmlFor` and `id`. Use React's `useId()` hook to generate collision-free IDs for reusable components.
