...
## 2026-06-26 - [Linking Labels in Custom Popovers]
**Learning:** Text acting as a label without semantic linking (`htmlFor` and `id`) fails to expand the click interaction area, forcing users to precision-click tiny numerical inputs in popovers like the inline calculator.
**Action:** Always use React's `useId()` hook to generate unique IDs, linking `<label>` elements to `<input>` fields.
