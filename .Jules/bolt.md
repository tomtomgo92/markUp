...
## 2026-04-19 - [Memoize Derived Financial Data]
**Learning:** When calculating complex financial derivatives (like mapping through scenarios or calculating progressive tax and margins), running these operations on every render blocks the main thread and causes UI lag during modal interactions or local state changes.
**Action:** Always wrap expensive derived data aggregations and mapping operations that rely on props in `useMemo` so they only recalculate when their specific data dependencies change.
