...
## 2026-04-07 - [Memoize Derived Financial Computations]
**Learning:** UI-only state toggles (like `activeCalculator`) passed down to complex components can cause cascading performance regressions if their derived data recalculations (such as mapped financial computations) are not memoized.
**Action:** Wrap expensive derived data calculations in `useMemo` so they only recalculate when their underlying data dependencies change.
