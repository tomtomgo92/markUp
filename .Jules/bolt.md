...
## 2026-04-09 - [Memoizing Derived Data for UI]
**Learning:** Generating derived data for charts or complex views (e.g., mapping through scenarios to run expensive computations like `calculateResults` inside `ComparisonView` and `ScenarioCard`) directly in the render path causes unnecessary recalculations on every state update (even unrelated ones like UI toggles).
**Action:** Always wrap derived data calculations in `useMemo` to prevent the operation from running on every render of the component, ensuring they only recalculate when their underlying data dependencies change.
