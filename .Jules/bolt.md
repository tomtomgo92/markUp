...
## 2026-03-05 - [Memoizing Derived Data with UI State Toggles]
**Learning:** When components combine complex derived data calculations (like aggregating financial results from a mapped array) with local UI state (like tracking an active popover or calculator), any change to the UI state triggers a full re-render, causing the expensive calculations to run unnecessarily.
**Action:** Always wrap complex derived data calculations in `useMemo` and depend strictly on their underlying data dependencies so they are not recalculated when independent UI states are toggled.
