...
## 2026-03-25 - [Memoizing Derived Financial Data]
**Learning:** To prevent cascading performance regressions, UI-only state toggles (like `isClientMode` or `activeCalculator`) passed down to complex components can trigger expensive derived data recalculations (such as mapped financial computations) on every render if not memoized.
**Action:** Use `useMemo` for expensive derived calculations (like `calculateResults` mappings) so they only recalculate when their underlying data dependencies change, protecting them from unrelated UI state updates.
