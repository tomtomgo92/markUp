...
const InputGroup = ({ label, value, onChange, icon: Icon, disabled, suffix, id }) => {
    const generatedId = useId();
    const inputId = id || generatedId;

    return (
    <div className={`space-y-2 ${disabled ? 'opacity-50 grayscale' : ''}`}>
        <label htmlFor={inputId} className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
            {label}
        </label>
