...
const ScenarioItemRow = memo(({ item, index, mode, onUpdate, onRemove, onOpenCalculator, onAddItem }) => {
    ...
    const marginHealth = itemMarginPercent >= 40 ? 'good' : itemMarginPercent >= 20 ? 'mid' : 'low';
        ...
        <tr className="scenario-row">
            ...
            <td className="scenario-row__margin-cell">
                <div className="scenario-row__margin">
                    <span className={`scenario-row__margin-value scenario-row__margin-value--${itemMargin >= 0 ? 'positive' : 'negative'}`}>
                        {itemMargin.toFixed(0)}€
                    </span>
                    <div className="scenario-row__margin-pct">
                        <div className={`scenario-row__health-dot scenario-row__health-dot--${marginHealth}`} title={`Santé de la marge: ${marginHealth === 'good' ? 'Excellente' : marginHealth === 'mid' ? 'Moyenne' : 'Critique'}`} aria-hidden="true" />
                        {(mode === 'cost_percent' || mode === 'pv_percent') ? (
                            ...
                            <span className="scenario-row__pct-static">{itemMarginPercent.toFixed(1)}%</span>
                        )}
                    </div>
                </div>
            </td>
