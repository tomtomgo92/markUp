...
const ComparisonView = ({ scenarios, onClose, isClientMode }) => {
    // BOLT: Optimize - use memo to avoid recalculating results for all scenarios
    // when comparison view is open and `isClientMode` is toggled
    const data = useMemo(() => {
    ...
    // BOLT: Optimize - memoize the maxPV computation
    const maxPV = useMemo(() => {
        return Math.max(...data.map(d => d.results.pv), 1);
    }, [data]); // Avoid div by 0
